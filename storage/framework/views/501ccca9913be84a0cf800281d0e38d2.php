    ...
    <div class="flex-1 lg:ml-72 h-screen flex flex-col bg-slate-50 overflow-hidden">
        ...
        <div class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-3">
                <a href="<?php echo e(route('teacher.announcements.index')); ?>" class="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
                    <i class="fas fa-arrow-left"></i>
                </a>
                <div>
                    <h1 class="text-xl font-bold text-slate-900">Announcement Details</h1>
                    <p class="text-sm text-slate-500 mt-0.5">Posted <?php echo e($announcement->created_at->diffForHumans()); ?></p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <a href="<?php echo e(route('teacher.announcements.edit', $announcement)); ?>" class="px-4 py-2 text-sm font-medium text-emerald-600 bg-emerald-50 rounded-xl hover:bg-emerald-100 transition-colors">
                    <i class="fas fa-edit mr-1"></i> Edit
                </a>
                <form action="<?php echo e(route('teacher.announcements.pin', $announcement)); ?>" method="POST" class="inline">
                    <?php echo csrf_field(); ?>
                    <button type="submit" class="px-4 py-2 text-sm font-medium <?php echo e($announcement->pinned ? 'text-amber-600 bg-amber-50' : 'text-slate-600 bg-slate-100'); ?> rounded-xl hover:bg-opacity-80 transition-colors">
                        <i class="fas fa-thumbtack mr-1"></i> <?php echo e($announcement->pinned ? 'Unpin' : 'Pin'); ?>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div class="p-6 space-y-3 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-slate-900"><?php echo e($announcement->created_at->format('M d, Y g:i A')); ?></span>
                        </div>
                        <?php if($announcement->expires_at): ?>
                            <div class="flex justify-between">
                                <span class="text-slate-500">Expires</span>
                                <span class="font-medium <?php echo e($announcement->expires_at->isPast() ? 'text-rose-600' : 'text-slate-900'); ?>"><?php echo e($announcement->expires_at->format('M d, Y g:i A')); ?></span>
                            </div>
                        <?php endif; ?>
                        <div class="flex justify-between">
                            <span class="text-slate-500">School Year</span>
                            <span class="font-medium text-slate-900"><?php echo e($announcement->schoolYear->name ?? 'N/A'); ?></span>
                        </div>
                    </div>
                </div>
            </div>
...
<form x-ref="deleteForm" action="<?php echo e(route('teacher.announcements.destroy', $announcement)); ?>" method="POST" class="hidden">
    <?php echo csrf_field(); ?>
...
<?php /**PATH C:\Program Files\Ampps\www\projects\capstone\tugaweesms-main\resources\views\teacher\announcements\show.blade.php ENDPATH**/ ?>