...
<div class="flex h-screen">
    ...
    <div class="flex-1 lg:ml-72 h-screen flex flex-col bg-slate-50 overflow-hidden">
        ...
        <div class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0">
            ...
            <a href="<?php echo e(route('teacher.announcements.create')); ?>" class="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-indigo-600 to-violet-600 rounded-xl hover:from-indigo-700 hover:to-violet-700 transition-all shadow-lg shadow-indigo-500/30">
        ...
        <div class="flex-1 overflow-y-auto p-6">
            ...
            <?php if($announcements->isEmpty()): ?>
                <div class="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <a href="<?php echo e(route('teacher.announcements.create')); ?>" class="mt-4 px-5 py-2.5 text-sm font-medium text-indigo-600 bg-indigo-50 rounded-xl hover:bg-indigo-100 transition-colors">
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-2 mb-1 flex-wrap">
                                            ...
                                            <?php if($announcement->sections->count() > 0): ?>
                                                <span class="px-2 py-0.5 bg-slate-100 text-slate-500 text-[10px] rounded-full" title="<?php echo e($announcement->sections->pluck('name')->join(', ')); ?>">
                                                    <?php echo e($announcement->sections->count()); ?> section<?php echo e($announcement->sections->count() > 1 ? 's' : ''); ?>

                                                </span>
                                            <?php endif; ?>
                                        </div>

                                        <h3 class="font-semibold text-slate-900 truncate"><?php echo e($announcement->title); ?></h3>
                                        ...
                                        <?php if($announcement->attachments->count() > 0): ?>
                                            <div class="flex flex-wrap gap-2 mt-3">
                                                <?php $__currentLoopData = $announcement->attachments->take(3); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $att): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <?php if($att->isImage()): ?>
                                                        <div @click="openLightbox('<?php echo e($att->url()); ?>')" class="relative w-16 h-16 rounded-lg overflow-hidden cursor-pointer group border border-slate-200">
                                                            <img src="<?php echo e($att->url()); ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform">
                                                            <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                                                                <i class="fas fa-expand text-white opacity-0 group-hover:opacity-100 text-xs"></i>
                                                            </div>
                                                        </div>
                                                    <?php else: ?>
                                                        <a href="<?php echo e($att->url()); ?>" download class="flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-600 hover:bg-slate-100 transition-colors">
                                                            <i class="fas fa-file text-slate-400"></i>
                                                            <span class="truncate max-w-[80px]"><?php echo e($att->file_name); ?></span>
                                                        </a>
                                                    <?php endif; ?>
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                <?php if($announcement->attachments->count() > 3): ?>
                                                    <span class="flex items-center px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-500">
                                                        +<?php echo e($announcement->attachments->count() - 3); ?> more
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                        <?php endif; ?>
                                    ...
                                    <div class="flex items-center gap-1 shrink-0">
                                        <a href="<?php echo e(route('teacher.announcements.show', $announcement)); ?>" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="View Details">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="<?php echo e(route('teacher.announcements.edit', $announcement)); ?>" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="<?php echo e(route('teacher.announcements.pin', $announcement)); ?>" method="POST" class="inline">
                                            <?php echo csrf_field(); ?>
...
function announcementList() {
    return {
        ...
        executeDelete() {
            ...
            form.setAttribute('action', '<?php echo e(url('/teacher/announcements')); ?>/' + this.deleteAnnouncementId);
            form.submit();
        },
...
<?php /**PATH C:\Program Files\Ampps\www\projects\capstone\tugaweesms-main\resources\views\teacher\announcements\index.blade.php ENDPATH**/ ?>