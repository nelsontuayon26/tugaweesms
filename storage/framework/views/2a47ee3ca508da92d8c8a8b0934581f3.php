...
<head>
    ...
    <title>Grades - <?php echo e($section->name); ?></title>
...
<div class="flex">
    ...
    <div class="lg:ml-72 w-full min-h-screen p-6">
        ...
        <div class="mb-6">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-0.5"><?php echo e($section->name); ?> &middot; <?php echo e($section->gradeLevel->name ?? 'N/A'); ?></p>
                </div>
                <div class="flex items-center gap-2 bg-white rounded-xl border border-slate-200 shadow-sm px-3 py-2">
                    ...
                    <select id="quarterSelect" class="bg-transparent text-sm font-medium text-slate-700 focus:outline-none cursor-pointer">
                        <option value="1" <?php echo e($requestedQuarter == 1 ? 'selected' : ''); ?>>1st Quarter</option>
                        <option value="2" <?php echo e($requestedQuarter == 2 ? 'selected' : ''); ?>>2nd Quarter</option>
                        <option value="3" <?php echo e($requestedQuarter == 3 ? 'selected' : ''); ?>>3rd Quarter</option>
                        <option value="4" <?php echo e($requestedQuarter == 4 ? 'selected' : ''); ?>>4th Quarter</option>
                    </select>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm">
                        Q<?php echo e($requestedQuarter); ?>

                    </div>
                    <div>
                        <p class="font-semibold text-slate-900"><?php echo e($quarterInfo->display_name); ?></p>
                        <p class="text-xs text-slate-500">
                            <?php echo e($quarterInfo->start_date?->format('M d, Y')); ?> — <?php echo e($quarterInfo->end_date?->format('M d, Y')); ?>

                            <?php if($quarterInfo->is_current): ?>
                            ...
                            <?php elseif($quarterInfo->is_past): ?>
                            ...
                            <?php elseif($quarterInfo->is_upcoming): ?>
                ...
                <?php if($quarterInfo->is_current && $quarterInfo->progress_percent): ?>
                <div class="flex items-center gap-3 min-w-[160px]">
                    <div class="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div class="h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all" style="width: <?php echo e($quarterInfo->progress_percent); ?>%"></div>
                    </div>
                    <span class="text-xs font-semibold text-slate-600 w-8 text-right"><?php echo e($quarterInfo->progress_percent); ?>%</span>
                </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-start gap-3">
            ...
            <div>
                <?php if($finalization?->grades_finalized): ?>
                    ...
                    <p class="text-xs text-amber-600 mt-1">Finalized on: <?php echo e($finalization->grades_finalized_at->format('F d, Y \a\t h:i A')); ?></p>
                    <?php endif; ?>
                <?php elseif($finalization?->is_locked): ?>
                ...
                <?php elseif($quarterInfo && !$quarterInfo->is_current): ?>
                    ...
                    <?php if($quarterInfo->is_upcoming): ?>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-slate-500">Choose a subject to manage grades for <span class="font-medium text-indigo-600"><?php echo e($section->gradeLevel->name ?? 'this section'); ?></span></p>
                </div>
                <form method="GET" class="flex flex-col sm:flex-row items-start sm:items-center gap-3" id="subjectForm">
                    <input type="hidden" name="quarter" id="quarterInput" value="<?php echo e($requestedQuarter); ?>">
                    <input type="hidden" name="grade_level" value="<?php echo e($section->grade_level_id); ?>">
                    ...
                    <button type="submit" id="loadGradesBtn"
                            ...
                            <?php echo e(!request('subject') ? 'disabled' : ''); ?>>Load Grades</button>
                </form>
            </div>
            <?php if(isset($selectedGradeLevel)): ?>
            <div class="mt-4 pt-4 border-t border-slate-100 flex flex-wrap gap-4 text-sm">
                <span class="text-slate-600"><span class="font-medium text-slate-900"><?php echo e($selectedGradeLevel->name); ?></span> &middot; <?php echo e($filteredSubjects->count()); ?> subjects &middot; <?php echo e($students->count()); ?> students</span>
            </div>
            <?php endif; ?>
        </div>

        <?php if(isset($selectedSubject)): ?>
        <form method="POST" action="<?php echo e(route('teacher.sections.grades.store', $section)); ?>" class="space-y-6" id="gradeForm">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="subject_id" value="<?php echo e($selectedSubject->id); ?>">
            <input type="hidden" name="grade_level_id" value="<?php echo e($selectedGradeLevel->id); ?>">
            <input type="hidden" name="quarter" value="<?php echo e($requestedQuarter); ?>">
            ...
            <div class="bg-indigo-600 rounded-xl shadow-sm p-5 text-white flex items-center justify-between">
                <div>
                    ...
                    <h2 class="text-xl font-bold"><?php echo e($selectedSubject->name); ?></h2>
                    <p class="text-indigo-200 text-sm"><?php echo e($selectedGradeLevel->name); ?> &middot; <?php echo e($section->name); ?> &middot; Quarter <?php echo e($requestedQuarter); ?></p>
                </div>
                <div class="text-right">
                    <p class="text-indigo-200 text-xs">Code</p>
                    <p class="text-lg font-bold"><?php echo e($selectedSubject->code ?? 'N/A'); ?></p>
                </div>
            </div>
                ...
                <button type="button" @click="open = !open" class="w-full px-5 py-4 flex items-center justify-between bg-slate-50 hover:bg-slate-100 transition-colors">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="flex gap-2 text-xs">
                            <span class="px-2 py-1 bg-blue-50 text-blue-700 rounded font-medium">WW <?php echo e($gradeWeights->ww_weight); ?>%</span>
                            <span class="px-2 py-1 bg-purple-50 text-purple-700 rounded font-medium">PT <?php echo e($gradeWeights->pt_weight); ?>%</span>
                            <span class="px-2 py-1 bg-amber-50 text-amber-700 rounded font-medium">QE <?php echo e($gradeWeights->qe_weight); ?>%</span>
                        </div>
                ...
                <div x-show="open" x-collapse class="border-t border-slate-100">
                    <div class="p-5 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" name="ww_weight" id="wwWeight" value="<?php echo e($gradeWeights->ww_weight); ?>" min="0" max="100"
                                   class="w-full px-4 py-2.5 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-semibold text-blue-600 <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>"
                                   onchange="validateWeights()" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Performance Task (%)</label>
                            <input type="number" name="pt_weight" id="ptWeight" value="<?php echo e($gradeWeights->pt_weight); ?>" min="0" max="100"
                                   class="w-full px-4 py-2.5 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 font-semibold text-purple-600 <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>"
                                   onchange="validateWeights()" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Quarterly Exam (%)</label>
                            <input type="number" name="qe_weight" id="qeWeight" value="<?php echo e($gradeWeights->qe_weight); ?>" min="0" max="100"
                                   class="w-full px-4 py-2.5 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 font-semibold text-amber-600 <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>"
                                   onchange="validateWeights()" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                        </div>
                    </div>
                    ...
                    <div class="px-5 pb-4">
                        <button type="button" onclick="resetWeights()" class="text-sm text-indigo-600 hover:text-indigo-700 font-medium <?php echo e(!$isEditable ? 'opacity-50 cursor-not-allowed' : ''); ?>" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>Reset to Default (40/40/20)</button>
                    </div>
                </div>
            </div>
            <!-- Tabs -->
            <div x-data="{ activeTab: 'ww' }" class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div x-show="activeTab === 'ww'" class="p-5">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex gap-2">
                            <button type="button" onclick="addWWColumn()" class="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5 <?php echo e(!$isEditable ? 'opacity-50 cursor-not-allowed' : ''); ?>" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                                <i class="fas fa-plus text-xs"></i> Add Activity
                            </button>
                            <button type="button" onclick="removeLastWW()" class="px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5 <?php echo e(!$isEditable ? 'opacity-50 cursor-not-allowed' : ''); ?>" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                        ...
                        <table class="w-full text-sm border-collapse" id="wwTable">
                            <thead>
                                <tr class="bg-slate-50 border-b border-slate-200" id="wwHeaderRow">
                                    ...
                                    <?php $wwTitles = $existingGrades['ww_titles'] ?? []; ?>
                                    <?php $wwTotalItems = $existingGrades['ww_total_items'] ?? []; ?>
                                    <?php for($i = 0; $i < max(3, count($wwTitles)); $i++): ?>
                                    <th class="px-2 py-2.5 text-center font-semibold text-slate-700 ww-col-header" data-col="<?php echo e($i + 1); ?>">
                                        <div class="mb-1 text-xs">WW <?php echo e($i + 1); ?></div>
                                        <input type="text" name="ww_titles[]" class="ww-title w-14 text-center bg-transparent border-b border-slate-300 text-xs mb-1 <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>" placeholder="Title" value="<?php echo e($wwTitles[$i] ?? ''); ?>" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                                        <input type="number" name="ww_total_items[]" class="ww-total-item w-14 text-center bg-slate-50 border border-slate-200 rounded text-xs py-0.5 <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>" placeholder="Items" value="<?php echo e($wwTotalItems[$i] ?? 100); ?>" min="1" onchange="calculateAllWW()" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                                    </th>
                                    <?php endfor; ?>
                            ...
                            <tbody class="divide-y divide-slate-100" id="wwTableBody">
                                ...
                                <tr class="hover:bg-slate-50/50 transition-colors" data-student-id="<?php echo e($student->id); ?>">
                                    <td class="px-3 py-2.5 text-slate-400 font-medium"><?php echo e($index + 1); ?></td>
                                    <td class="px-3 py-2.5 font-medium text-slate-900"><?php echo e($student->user->last_name); ?>, <?php echo e($student->user->first_name); ?></td>
                                    <?php $wwKey = $student->id . '_written_work'; ?>
                                    <?php $wwScores = $existingGrades[$wwKey]['scores'] ?? []; ?>
                                    <?php for($i = 0; $i < max(3, count($wwTitles)); $i++): ?>
                                    <td class="px-2 py-2.5 text-center ww-col" data-col="<?php echo e($i + 1); ?>">
                                        <input type="number" name="ww[<?php echo e($student->id); ?>][]" class="ww-score w-16 px-2 py-1.5 text-center rounded-md border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-all <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>" min="0" value="<?php echo e($wwScores[$i] ?? ''); ?>" onchange="calculateStudentWW(<?php echo e($student->id); ?>)" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                                    </td>
                                    <?php endfor; ?>
                                    <td class="px-3 py-2.5 text-center font-semibold text-slate-700"><span class="ww-total" id="ww-total-<?php echo e($student->id); ?>">0</span></td>
                                    <td class="px-3 py-2.5 text-center"><span class="ww-ps px-2 py-0.5 rounded bg-blue-50 text-blue-700 font-semibold text-xs" id="ww-ps-<?php echo e($student->id); ?>">0.00</span></td>
                                    <td class="px-3 py-2.5 text-center"><span class="ww-ws px-2 py-0.5 rounded bg-blue-100 text-blue-800 font-bold text-xs" id="ww-ws-<?php echo e($student->id); ?>">0.00</span></td>
                                </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </tbody>
                        </table>
                ...
                <div x-show="activeTab === 'pt'" class="p-5" x-cloak>
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex gap-2">
                            <button type="button" onclick="addPTColumn()" class="px-3 py-1.5 bg-purple-50 hover:bg-purple-100 text-purple-700 rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5 <?php echo e(!$isEditable ? 'opacity-50 cursor-not-allowed' : ''); ?>" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                                <i class="fas fa-plus text-xs"></i> Add Task
                            </button>
                            <button type="button" onclick="removeLastPT()" class="px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5 <?php echo e(!$isEditable ? 'opacity-50 cursor-not-allowed' : ''); ?>" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                                <i class="fas fa-minus text-xs"></i> Remove Last
                            </button>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-sm border-collapse" id="ptTable">
                            <thead>
                                <tr class="bg-slate-50 border-b border-slate-200" id="ptHeaderRow">
                                    <th class="px-3 py-2.5 text-left font-semibold text-slate-700 w-10">#</th>
                                    <th class="px-3 py-2.5 text-left font-semibold text-slate-700">Student Name</th>
                                    <?php $ptTitles = $existingGrades['pt_titles'] ?? []; ?>
                                    <?php $ptTotalItems = $existingGrades['pt_total_items'] ?? []; ?>
                                    <?php for($i = 0; $i < max(2, count($ptTitles)); $i++): ?>
                                    <th class="px-2 py-2.5 text-center font-semibold text-slate-700 pt-col-header" data-col="<?php echo e($i + 1); ?>">
                                        <div class="mb-1 text-xs">PT <?php echo e($i + 1); ?></div>
                                        <input type="text" name="pt_titles[]" class="pt-title w-14 text-center bg-transparent border-b border-slate-300 text-xs mb-1 <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>" placeholder="Title" value="<?php echo e($ptTitles[$i] ?? ''); ?>" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                                        <input type="number" name="pt_total_items[]" class="pt-total-item w-14 text-center bg-slate-50 border border-slate-200 rounded text-xs py-0.5 <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>" placeholder="Items" value="<?php echo e($ptTotalItems[$i] ?? 100); ?>" min="1" onchange="calculateAllPT()" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                                    </th>
                                    <?php endfor; ?>
                            ...
                            <tbody class="divide-y divide-slate-100" id="ptTableBody">
                                ...
                                <tr class="hover:bg-slate-50/50 transition-colors" data-student-id="<?php echo e($student->id); ?>">
                                    <td class="px-3 py-2.5 text-slate-400 font-medium"><?php echo e($index + 1); ?></td>
                                    <td class="px-3 py-2.5 font-medium text-slate-900"><?php echo e($student->user->last_name); ?>, <?php echo e($student->user->first_name); ?></td>
                                    <?php $ptKey = $student->id . '_performance_task'; ?>
                                    <?php $ptScores = $existingGrades[$ptKey]['scores'] ?? []; ?>
                                    <?php for($i = 0; $i < max(2, count($ptTitles)); $i++): ?>
                                    <td class="px-2 py-2.5 text-center pt-col" data-col="<?php echo e($i + 1); ?>">
                                        <input type="number" name="pt[<?php echo e($student->id); ?>][]" class="pt-score w-16 px-2 py-1.5 text-center rounded-md border border-slate-200 focus:border-purple-500 focus:ring-1 focus:ring-purple-500/20 transition-all <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>" min="0" value="<?php echo e($ptScores[$i] ?? ''); ?>" onchange="calculateStudentPT(<?php echo e($student->id); ?>)" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                                    </td>
                                    <?php endfor; ?>
                                    <td class="px-3 py-2.5 text-center font-semibold text-slate-700"><span class="pt-total" id="pt-total-<?php echo e($student->id); ?>">0</span></td>
                                    <td class="px-3 py-2.5 text-center"><span class="pt-ps px-2 py-0.5 rounded bg-purple-50 text-purple-700 font-semibold text-xs" id="pt-ps-<?php echo e($student->id); ?>">0.00</span></td>
                                    <td class="px-3 py-2.5 text-center"><span class="pt-ws px-2 py-0.5 rounded bg-purple-100 text-purple-800 font-bold text-xs" id="pt-ws-<?php echo e($student->id); ?>">0.00</span></td>
                                </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </tbody>
                        </table>
                ...
                <div x-show="activeTab === 'qe'" class="p-5" x-cloak>
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="hidden" name="qe_total_items" id="qeTotalItemsInput" value="<?php echo e($existingGrades['qe_total_items'] ?? 100); ?>">
                            <input type="number" id="qeTotal" value="<?php echo e($existingGrades['qe_total_items'] ?? 100); ?>" class="w-20 px-3 py-1.5 rounded-md border border-slate-200 text-sm font-semibold text-amber-600 <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>" onchange="updateQETotalItems(this.value)" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                        </div>
                    </div>
                    <div class="overflow-x-auto">
                            ...
                            <tbody class="divide-y divide-slate-100" id="qeTableBody">
                                ...
                                <tr class="hover:bg-slate-50/50 transition-colors" data-student-id="<?php echo e($student->id); ?>">
                                    ...
                                    <td class="px-3 py-2.5 text-center">
                                        <?php $qeKey = $student->id . '_quarterly_exam'; ?>
                                        <input type="number" name="qe[<?php echo e($student->id); ?>]" class="qe-score w-20 px-3 py-1.5 text-center rounded-md border border-slate-200 focus:border-amber-500 focus:ring-1 focus:ring-amber-500/20 transition-all font-semibold <?php echo e(!$isEditable ? 'bg-slate-100 cursor-not-allowed' : ''); ?>" min="0" value="<?php echo e($existingGrades[$qeKey]['total_score'] ?? ''); ?>" onchange="calculateStudentQE(<?php echo e($student->id); ?>)" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                                    </td>
                                    <td class="px-3 py-2.5 text-center"><span class="qe-ps px-2 py-0.5 rounded bg-amber-50 text-amber-700 font-semibold text-xs" id="qe-ps-<?php echo e($student->id); ?>">0.00</span></td>
                                    <td class="px-3 py-2.5 text-center"><span class="qe-ws px-2 py-0.5 rounded bg-amber-100 text-amber-800 font-bold text-xs" id="qe-ws-<?php echo e($student->id); ?>">0.00</span></td>
                                </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </tbody>
                        </table>
                        ...
                        <table class="w-full min-w-[700px] text-sm border-collapse" id="summaryTable">
                            <thead>
                                <tr class="bg-slate-50 border-b border-slate-200">
                                    ...
                                    <th class="px-3 py-2.5 text-center font-semibold text-slate-700">WW (<?php echo e($gradeWeights->ww_weight); ?>%)</th>
                                    <th class="px-3 py-2.5 text-center font-semibold text-slate-700">PT (<?php echo e($gradeWeights->pt_weight); ?>%)</th>
                                    <th class="px-3 py-2.5 text-center font-semibold text-slate-700">QE (<?php echo e($gradeWeights->qe_weight); ?>%)</th>
                            ...
                            <tbody class="divide-y divide-slate-100" id="summaryTableBody">
                                ...
                                <tr class="hover:bg-slate-50/50 transition-colors" data-student-id="<?php echo e($student->id); ?>">
                                    <td class="px-3 py-2.5 font-medium text-slate-900"><?php echo e($student->user->last_name); ?>, <?php echo e($student->user->first_name); ?></td>
                                    <td class="px-3 py-2.5 text-center"><span class="final-ww" id="final-ww-<?php echo e($student->id); ?>">0.00</span></td>
                                    <td class="px-3 py-2.5 text-center"><span class="final-pt" id="final-pt-<?php echo e($student->id); ?>">0.00</span></td>
                                    <td class="px-3 py-2.5 text-center"><span class="final-qe" id="final-qe-<?php echo e($student->id); ?>">0.00</span></td>
                                    <td class="px-3 py-2.5 text-center font-semibold"><span class="initial-grade" id="initial-grade-<?php echo e($student->id); ?>">0.00</span></td>
                                    <td class="px-3 py-2.5 text-center bg-slate-50/50"><span class="transmuted-grade text-lg font-bold" id="transmuted-<?php echo e($student->id); ?>">0.00</span></td>
                                    <td class="px-3 py-2.5 text-center"><span class="remarks px-2 py-0.5 rounded-full text-xs font-bold" id="remarks-<?php echo e($student->id); ?>">-</span></td>
                                </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </tbody>
                        </table>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white rounded-xl border border-slate-200 shadow-sm p-4">
                ...
                <div class="flex gap-3">
                    <button type="button" onclick="calculateAllGrades()" class="px-5 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg font-medium transition-colors flex items-center gap-2 <?php echo e(!$isEditable ? 'opacity-50 cursor-not-allowed' : ''); ?>" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
                        <i class="fas fa-sync-alt text-sm"></i> Recalculate
                    </button>
                    <button type="submit" class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-medium transition-colors flex items-center gap-2 <?php echo e(!$isEditable ? 'opacity-50 cursor-not-allowed' : ''); ?>" <?php echo e(!$isEditable ? 'disabled' : ''); ?>>
        ...
        <?php if($isAdviser && $isEditable && !$finalization?->grades_finalized): ?>
            ...
            <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full mx-4">
                ...
                <div class="p-5">
                    <div class="bg-slate-50 rounded-lg p-3 mb-3 text-sm text-slate-700">
                        You are about to finalize grades for <strong><?php echo e($section->name); ?></strong>.
                    </div>
                    <form id="finalizeGradesForm" action="<?php echo e(route('teacher.sections.grades.finalize', $section)); ?>" method="POST">
                        <?php echo csrf_field(); ?>
        ...
        <?php elseif($finalization?->grades_finalized): ?>
        <div class="mt-6 bg-emerald-50 rounded-xl border border-emerald-200 p-5 flex items-start gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-emerald-700">Grades were finalized on <?php echo e($finalization->grades_finalized_at?->format('F d, Y \a\t h:i A')); ?>.</p>
            </div>
        </div>
        <?php endif; ?>
...
<script>
    const existingGrades = <?php echo json_encode($existingGrades ?? [], 15, 512) ?>;
    ...
    document.addEventListener('DOMContentLoaded',function(){
        ...
        <?php if(session('error')): ?>
            const err='<?php echo e(session('error')); ?>';
...
<?php /**PATH C:\Program Files\Ampps\www\projects\capstone\tugaweesms-main\resources\views\teacher\grades\index.blade.php ENDPATH**/ ?>