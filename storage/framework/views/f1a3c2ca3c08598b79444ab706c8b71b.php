...
<head>
    ...
    <title>Messenger - <?php echo e($isStudent ? 'Pupil' : 'Teacher'); ?> Portal</title>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <select x-model="newConversation.recipientId" required
                                ...
                                <?php if($isStudent): ?>
                                    <?php if($section && $section->teacher): ?>
                                        <option value="<?php echo e($section->teacher->user_id); ?>">
                                            <?php echo e($section->teacher->user->full_name); ?> (Adviser)
                                        </option>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <?php $__currentLoopData = auth()->user()->teacher?->sections ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $section): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
function messenger() {
    return {
        ...
        async loadConversations() {
            try {
                const response = await fetch('<?php echo e(route('api.conversations.index')); ?>');
        ...
        async sendMessage() {
            ...
            try {
                const response = await fetch('<?php echo e(route('api.messages.store')); ?>', {
                    method: 'POST',
                    headers: {
...
<?php /**PATH C:\Program Files\Ampps\www\projects\capstone\tugaweesms-main\resources\views\components\messenger.blade.php ENDPATH**/ ?>