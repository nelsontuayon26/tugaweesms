...
<?php $__env->startSection('header-title', 'Edit Announcement'); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-3xl mx-auto" x-data="announcementForm()">
    <form action="<?php echo e(route('admin.announcements.update', $announcement)); ?>" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" @submit="return validateForm()">
        <?php echo csrf_field(); ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="target" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm bg-white">
                        <option value="students" <?php echo e($announcement->target === 'students' ? 'selected' : ''); ?>>Pupils</option>
                        <option value="teachers" <?php echo e($announcement->target === 'teachers' ? 'selected' : ''); ?>>Teachers</option>
                        <option value="all" <?php echo e($announcement->target === 'all' ? 'selected' : ''); ?>>Teachers & Pupils</option>
                    </select>
                </div>
        ...
        <div class="p-6 bg-slate-50 flex items-center justify-end gap-3">
            <a href="<?php echo e(route('admin.announcements.index')); ?>" class="px-6 py-2.5 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">Cancel</a>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Program Files\Ampps\www\projects\capstone\tugaweesms-main\resources\views\admin\announcements\edit.blade.php ENDPATH**/ ?>