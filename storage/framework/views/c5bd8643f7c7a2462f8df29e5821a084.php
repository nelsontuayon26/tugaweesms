...
<div class="max-w-7xl mx-auto" x-data="enrollmentApp()">
    ...
    <div class="flex justify-between items-start mb-6">
        <div>
            <div class="flex items-center gap-3 mb-2">
                <h1 class="text-2xl font-bold text-slate-800"><?php echo e($application->application_number); ?></h1>
            ...
            <p class="text-slate-500">Submitted on <?php echo e($application->created_at->format('F d, Y \a\t h:i A')); ?></p>
        </div>
        <a href="<?php echo e(route('admin.enrollment.index')); ?>" class="px-4 py-2 bg-white text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl border border-slate-200 transition-all">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="p-3 bg-slate-50 rounded-xl">
                        ...
                        <p class="font-semibold text-slate-800"><?php echo e($application->student_full_name); ?></p>
                    </div>
                    <div class="p-3 bg-slate-50 rounded-xl">
                        <p class="text-xs text-slate-500 uppercase">LRN</p>
                        <p class="font-semibold text-slate-800"><?php echo e($application->student_lrn ?? 'N/A'); ?></p>
                    </div>
                    <div class="p-3 bg-slate-50 rounded-xl">
                        <p class="text-xs text-slate-500 uppercase">Current Grade (Next Year)</p>
                        <p class="font-semibold text-slate-800"><?php echo e($application->gradeLevel->name ?? 'N/A'); ?></p>
                    </div>
                    <div class="p-3 bg-slate-50 rounded-xl">
                        <p class="text-xs text-slate-500 uppercase">School Year</p>
                        <p class="font-semibold text-slate-800"><?php echo e($application->schoolYear->name ?? 'N/A'); ?></p>
                    </div>
                    <?php if($existingStudent): ?>
                    <div class="p-3 bg-slate-50 rounded-xl">
                        <p class="text-xs text-slate-500 uppercase">Current Section</p>
                        <p class="font-semibold text-slate-800"><?php echo e($existingStudent->section->name ?? 'Not assigned'); ?></p>
                    </div>
                    <div class="p-3 bg-slate-50 rounded-xl">
                        <p class="text-xs text-slate-500 uppercase">Student ID</p>
                        <p class="font-semibold text-slate-800"><?php echo e($existingStudent->id); ?></p>
                    </div>
                    <?php endif; ?>
                    <div class="p-3 bg-slate-50 rounded-xl">
                        <p class="text-xs text-slate-500 uppercase">Parent Email</p>
                        <p class="font-semibold text-slate-800"><?php echo e($application->parent_email); ?></p>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-sm">
                        ...
                        <span class="font-semibold <?php echo e($application->documents->where('status', 'verified')->count() === $application->documents->count() && $application->documents->count() > 0 ? 'text-emerald-600' : 'text-amber-600'); ?>">
                            <?php echo e($application->documents->where('status', 'verified')->count()); ?> / <?php echo e($application->documents->count()); ?>
                ...
                <div class="space-y-3">
                    <?php $__empty_1 = true; $__currentLoopData = $application->documents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $doc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl border <?php echo e($doc->status === 'verified' ? 'border-emerald-200 bg-emerald-50/30' : ($doc->status === 'rejected' ? 'border-red-200 bg-red-50/30' : 'border-slate-200')); ?>">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-lg <?php echo e($doc->isPdf() ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'); ?> flex items-center justify-center">
                                    <i class="fas <?php echo e($doc->isPdf() ? 'fa-file-pdf' : 'fa-file-image'); ?> text-lg"></i>
                                </div>
                                <div>
                                    <p class="font-semibold text-slate-800"><?php echo e($doc->document_name); ?></p>
                                    <p class="text-xs text-slate-500"><?php echo e($doc->formatted_file_size); ?></p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="px-2 py-1 rounded-lg text-xs font-medium
                                    <?php if($doc->status == 'verified'): ?> bg-emerald-100 text-emerald-700
                                    <?php elseif($doc->status == 'rejected'): ?> bg-red-100 text-red-700
                                    <?php else: ?> bg-amber-100 text-amber-700 <?php endif; ?>">
                                    <i class="fas <?php echo e($doc->status == 'verified' ? 'fa-check' : ($doc->status == 'rejected' ? 'fa-times' : 'fa-clock')); ?> mr-1"></i>
                                    <?php echo e(ucfirst($doc->status)); ?>

                                </span>
                                <button type="button" 
                                        @click="openDocumentModal('<?php echo e($doc->file_url); ?>', '<?php echo e($doc->document_name); ?>', '<?php echo e($doc->file_type); ?>')"
                                ...
                                <form action="<?php echo e(route('admin.enrollment.verify-document', $doc)); ?>" method="POST" class="inline">
                                    <?php echo csrf_field(); ?>
                    ...
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <div class="text-center py-8 text-slate-400">
                            ...
                            <?php if($application->application_type === 'continuing'): ?>
                ...
                <?php if($application->documents->isNotEmpty() && $application->application_type !== 'continuing'): ?>
            ...
            <!-- Admin Notes -->
            <?php if($application->admin_notes || $application->rejection_reason): ?>
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <?php if($application->admin_notes): ?>
                    <div class="p-3 bg-slate-50 rounded-xl mb-3">
                        <p class="text-sm text-slate-600"><?php echo e($application->admin_notes); ?></p>
                    </div>
                <?php endif; ?>
                <?php if($application->rejection_reason): ?>
                    <div class="p-3 bg-red-50 border border-red-100 rounded-xl">
                        ...
                        <p class="text-sm text-red-600"><?php echo e($application->rejection_reason); ?></p>
                    </div>
                <?php endif; ?>
            </div>
            <?php endif; ?>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div class="flex justify-between p-2">
                        ...
                        <span class="font-medium"><?php echo e($application->application_type_label); ?></span>
                    </div>
                    <?php if($application->reviewed_by): ?>
                    <div class="pt-3 border-t border-slate-100">
                        <p class="text-xs text-slate-500">Reviewed By</p>
                        <p class="font-medium"><?php echo e($application->reviewer->name ?? 'N/A'); ?></p>
                        <p class="text-xs text-slate-400"><?php echo e($application->reviewed_at?->format('M d, Y h:i A')); ?></p>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <?php if($application->status !== 'approved' && $application->status !== 'rejected'): ?>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div class="p-3 bg-blue-50 border border-blue-200 rounded-xl mb-4">
                        <p class="text-sm text-blue-700">
                            <i class="fas fa-info-circle mr-1"></i>
                            This will enroll the continuing student in a new section for <?php echo e($application->schoolYear->name ?? 'the upcoming school year'); ?>.
                        </p>
                    </div>

                    <form action="<?php echo e(route('admin.enrollment.approve', $application)); ?>" method="POST" class="space-y-4" id="approveForm">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="school_year_id" value="<?php echo e($application->school_year_id); ?>">
                        <div>
                            ...
                            <select name="section_id" required class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" id="sectionSelect">
                                ...
                                <?php $__currentLoopData = $sections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $section): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($section->id); ?>" data-capacity="<?php echo e($section->enrollments_count >= $section->capacity ? 'full' : 'available'); ?>">
                            ...
                            <p class="text-xs text-slate-500 mt-1">Enrolling in <?php echo e($application->schoolYear->name ?? 'current school year'); ?></p>
                        </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    <form action="<?php echo e(route('admin.enrollment.reject', $application)); ?>" method="POST" class="space-y-4">
                        <?php echo csrf_field(); ?>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">
            ...
            <?php else: ?>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    <h3 class="font-bold text-slate-800 mb-4">Application <?php echo e(ucfirst($application->status)); ?></h3>
                    <div class="text-center py-4">
                        <div class="w-16 h-16 rounded-full <?php echo e($application->status === 'approved' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'); ?> flex items-center justify-center mx-auto mb-3">
                            <i class="fas <?php echo e($application->status === 'approved' ? 'fa-check' : 'fa-times'); ?> text-2xl"></i>
                        </div>
                        <p class="font-medium text-slate-800">
                            This application has been <?php echo e($application->status); ?>.
                        </p>
                        <?php if($application->status === 'approved' && $application->student_id): ?>
                            <a href="<?php echo e(route('admin.students.show', $application->student_id)); ?>" class="mt-3 inline-block px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Program Files\Ampps\www\projects\capstone\tugaweesms-main\resources\views\enrollment\admin\show.blade.php ENDPATH**/ ?>