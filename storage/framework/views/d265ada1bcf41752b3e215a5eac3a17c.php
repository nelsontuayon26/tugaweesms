        ...
        <header class="sticky top-0 z-20 bg-white border-b border-slate-200">
            <div class="flex items-center justify-between h-14 px-4 lg:px-6">
                ...
                <?php if($unreadCount > 0): ?>
                    <span class="bg-indigo-600 text-white text-xs font-medium px-2 py-1 rounded-full"><?php echo e($unreadCount); ?></span>
                <?php endif; ?>
            </div>
        </header>
        ...
        <div class="flex h-[calc(100vh-56px)]">
            ...
            <div class="w-full max-w-md bg-white border-r border-slate-200 flex flex-col">
                ...
                <div class="p-3 border-b border-slate-200 space-y-2">
                    ...
                    <a href="<?php echo e(route('student.messages.create')); ?>" 
                       class="flex items-center justify-center gap-2 w-full py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-all">
                        ...
                        Message <?php echo e($section && $section->teacher ? $section->teacher->user->first_name : 'Teacher'); ?>
                ...
                <div class="flex border-b border-slate-200">
                    <a href="<?php echo e(route('student.messages.index', ['tab' => 'inbox'])); ?>" 
                       class="flex-1 py-3 text-center text-sm font-medium <?php echo e($tab === 'inbox' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500 hover:text-slate-700'); ?>">
                        Inbox
                        <?php if($unreadCount > 0): ?>
                            <span class="ml-1 bg-rose-500 text-white text-[10px] px-1.5 rounded-full"><?php echo e($unreadCount); ?></span>
                        <?php endif; ?>
                    </a>
                    <a href="<?php echo e(route('student.messages.index', ['tab' => 'sent'])); ?>" 
                       class="flex-1 py-3 text-center text-sm font-medium <?php echo e($tab === 'sent' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500 hover:text-slate-700'); ?>">
                        Sent
                    </a>
                </div>
                ...
                <div class="flex-1 overflow-y-auto scrollbar-thin">
                    <?php if($messages && $messages->count() > 0): ?>
                        <?php $__currentLoopData = $messages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $message): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            ?>
                            <a href="<?php echo e(route('student.messages.show', $message)); ?>" 
                               class="message-item flex items-center gap-3 p-4 border-b border-slate-100 <?php echo e($isUnread ? 'unread' : ''); ?>">
                                
                                <!-- Avatar -->
                                <div class="flex-shrink-0">
                                    <div class="w-10 h-10 rounded-full <?php echo e($isSender ? 'bg-emerald-500' : 'bg-indigo-500'); ?> flex items-center justify-center text-white font-semibold text-sm">
                                        <?php echo e($otherPerson ? strtoupper(substr($otherPerson->first_name ?? 'U', 0, 1)) : '?'); ?>
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex justify-between items-baseline mb-0.5">
                                        <h3 class="font-semibold text-sm <?php echo e($isUnread ? 'text-slate-900' : 'text-slate-700'); ?> truncate">
                                            <?php echo e($otherPerson ? $otherPerson->full_name : 'Unknown'); ?>

                                            <?php if($isSender): ?>
                                                <span class="text-xs text-emerald-600 font-normal ml-1">(You sent)</span>
                                            <?php endif; ?>
                                        </h3>
                                        <span class="text-xs text-slate-400"><?php echo e($message->created_at->diffForHumans(null, true)); ?></span>
                                    </div>
                                    <p class="text-sm <?php echo e($isUnread ? 'font-semibold text-slate-800' : 'text-slate-600'); ?> truncate">
                                        <?php echo e($message->subject); ?>

                                    </p>
                                    <p class="text-xs text-slate-500 truncate"><?php echo e($message->preview); ?></p>
                                </div>
...
<?php /**PATH C:\Program Files\Ampps\www\projects\capstone\tugaweesms-main\resources\views\student\messages\index.blade.php ENDPATH**/ ?>