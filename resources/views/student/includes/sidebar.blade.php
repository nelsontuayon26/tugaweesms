...
<aside id="sidebar"
    ...
    <nav class="flex-1 px-0 py-2 space-y-0.5 overflow-y-auto"
        ...
        <a href="{{ route('student.dashboard') }}"
           class="student-nav-item {{ request()->routeIs('student.dashboard') ? 'active' : '' }}">
        ...
        <a href="{{ route('student.subjects') }}"
           class="student-nav-item {{ request()->routeIs('student.subjects*') ? 'active' : '' }}">
            <div class="student-nav-icon">
        ...
        <a href="{{ route('student.attendance') }}"
           class="student-nav-item {{ request()->routeIs('student.attendance*') ? 'active' : '' }}">
            <div class="student-nav-icon">
        ...
        <a href="{{ route('student.grades') }}"
           class="student-nav-item {{ request()->routeIs('student.grades*') ? 'active' : '' }}">
            <div class="student-nav-icon">
        ...
        <a href="{{ route('student.books') }}"
           class="student-nav-item {{ request()->routeIs('student.books*') ? 'active' : '' }}">
            <div class="student-nav-icon">
        ...
        <a href="{{ route('student.enrollment.index') }}"
           class="student-nav-item {{ request()->routeIs('student.enrollment*') ? 'active' : '' }}">
            <div class="student-nav-icon">
        ...
        <a href="{{ route('student.messenger') }}"
           class="student-nav-item {{ request()->routeIs('student.messenger') ? 'active' : '' }}">
            <div class="student-nav-icon">
            ...
            @endphp
            @if($unreadMessages > 0)
                <span class="ml-auto text-[10px] font-bold text-rose-400 bg-rose-900/30 px-2 py-0.5 rounded-md hide-on-collapse">{{ $unreadMessages > 9 ? '9+' : $unreadMessages }}</span>
            @endif
        </a>

        <a href="{{ route('student.announcements') }}"
           class="student-nav-item {{ request()->routeIs('student.announcements*') ? 'active' : '' }}">
            <div class="student-nav-icon">
                <i class="fas fa-bullhorn"></i>
            </div>
            <span class="hide-on-collapse">Announcements</span>
            @if($unreadAnnouncements > 0)
                <span class="ml-auto text-[10px] font-bold text-rose-400 bg-rose-900/30 px-2 py-0.5 rounded-md hide-on-collapse">{{ $unreadAnnouncements > 9 ? '9+' : $unreadAnnouncements }}</span>
            @endif
        </a>

        <a href="{{ route('student.events.index') }}"
           class="student-nav-item {{ request()->routeIs('student.events*') ? 'active' : '' }}">
            <div class="student-nav-icon">
    ...
    <div class="p-3">
        <div class="student-user-card" onclick="document.getElementById('studentUserMenu').classList.toggle('hidden')">
            <div class="flex items-center gap-3">
                @if($currentStudent && $currentStudent->user && $currentStudent->user->photo)
                    <img src="{{ profile_photo_url($currentStudent->user->photo) }}" alt="" class="w-9 h-9 rounded-full border-2 border-amber-500/30 object-cover flex-shrink-0">
                @else
                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                        {{ strtoupper(substr($currentStudent->user->first_name ?? 'S', 0, 1)) }}
                    </div>
                @endif
                <div class="flex-1 min-w-0 hide-on-collapse">
                    <p class="text-sm font-semibold text-white truncate">{{ $currentStudent->user->full_name ?? 'Student' }}</p>
                    <p class="text-[10px] text-amber-400 truncate">{{ $currentStudent->gradeLevel->name ?? 'Pupil' }}</p>
                </div>
            ...
            <div id="studentUserMenu" class="hidden mt-3 pt-3 border-t border-white/5 space-y-1 hide-on-collapse">
                <a href="{{ route('student.profile') }}" class="flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-white/5 transition-colors">
                    <i class="fas fa-user text-xs w-4"></i> Profile
                </a>
                <a href="{{ route('student.help') }}" class="flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-white/5 transition-colors">
