            ...
            <div class="p-card p-5">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-stone-900">{{ $teachers->count() }}</p>
                    </div>
            ...
            <div class="p-card p-5">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-stone-900">{{ $teachers->where('status', 'active')->count() }}</p>
                    </div>
            ...
            <div class="p-card p-5">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-stone-900">{{ $teachers->where('status', 'on_leave')->count() }}</p>
                    </div>
                    ...
                    <tbody>
                        @forelse($teachers as $teacher)
                            <tr>
                                <td>
                                    <div class="flex items-center gap-3">
                                        <img src="{{ profile_photo_url($teacher->user->photo) ?? 'https://ui-avatars.com/api/?name=' . urlencode(($teacher->user->first_name ?? $teacher->first_name) . '+' . ($teacher->user->last_name ?? $teacher->last_name)) . '&background=f5f5f4&color=57534e' }}"
                                             alt="" class="w-9 h-9 rounded-full border border-stone-200 object-cover">
                                        <div>
                                            <p class="font-bold text-stone-900 text-sm">{{ $teacher->first_name }} {{ $teacher->last_name }}</p>
                                            <p class="text-[10px] text-stone-400">{{ $teacher->user->username ?? '@teacher' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span class="text-sm text-stone-600">{{ $teacher->user->email ?? 'N/A' }}</span>
                                </td>
                                <td>
                                    <span class="text-sm text-stone-600">{{ $teacher->mobile_number ?? $teacher->telephone_number ?? 'N/A' }}</span>
                                </td>
                                <td>
                                    <span class="inline-flex items-center px-2 py-1 rounded-lg bg-purple-50 text-purple-700 font-semibold text-xs border border-purple-100">
                                        {{ $teacher->sections->count() ?? 0 }} section(s)
                                    </span>
                                </td>
                                <td>
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold {{ ($teacher->status ?? 'active') === 'active' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-stone-100 text-stone-600 border border-stone-200' }}">
