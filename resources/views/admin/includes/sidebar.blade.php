...
{{-- If the current user is a Principal, show the Principal sidebar instead --}}
@php
...
<aside id="sidebar"
    ...
    <div class="p-4 lg:p-6 border-b border-slate-100 flex-shrink-0">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 lg:w-12 lg:h-12 rounded-2xl flex items-center justify-center flex-shrink-0">
                <img src="{{ asset('images/logo.png') }}" alt="Tugawe Elementary School Logo" class="w-full h-full object-contain">
            </div>
            <div class="min-w-0">
    ...
    <nav class="flex-1 px-3 lg:px-4 py-4 lg:py-6 space-y-1 overflow-y-auto sidebar-nav custom-scrollbar"
        ...
        <a href="{{ route('admin.teacher-subject-assignments.index') }}" 
           class="nav-item {{ request()->routeIs('admin.teacher-subject-assignments.*') ? 'active' : '' }} flex items-center gap-3 px-3 lg:px-4 py-3 rounded-xl font-medium transition-all group {{ request()->routeIs('admin.teacher-subject-assignments.*') ? 'text-indigo-600 bg-indigo-50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}">
            <div class="w-8 h-8 rounded-lg {{ request()->routeIs('admin.teacher-subject-assignments.*') ? 'bg-indigo-100' : 'bg-slate-100 group-hover:bg-indigo-50' }} flex items-center justify-center flex-shrink-0 transition-colors">
                <i class="fas fa-book-open text-sm {{ request()->routeIs('admin.teacher-subject-assignments.*') ? 'text-indigo-600' : 'group-hover:text-indigo-600' }}"></i>
            </div>
        ...
        <div class="p-3 lg:p-4 border-t border-slate-100 user-menu-container">
            <div class="flex items-center gap-3 p-3 rounded-2xl bg-gradient-to-r from-slate-50 to-white border border-slate-200 shadow-sm cursor-pointer" onclick="toggleUserMenu()">
                <div class="relative flex-shrink-0">
                    @if(auth()->user()->photo)
                        <img src="{{ profile_photo_url(auth()->user()->photo) }}" 
                             alt="Admin" 
            ...
            <div id="userMenu" class="hidden bg-white rounded-2xl shadow-2xl border border-slate-200 p-2 w-56">
                ...
                <div class="p-1 space-y-0.5">
                    ...
                    <a href="{{ route('admin.activity-logs.index') }}"  class="flex items-center gap-3 px-3 py-2.5 text-sm text-slate-700 hover:bg-slate-50 rounded-xl transition-all group">
                    ...
                    <a href="{{ route('admin.import.students') }}"  class="flex items-center gap-3 px-3 py-2.5 text-sm text-slate-700 hover:bg-slate-50 rounded-xl transition-all group">
                    ...
                    <a href="{{ route('pwa.settings') }}" class="flex items-center gap-3 px-3 py-2.5 text-sm text-slate-700 hover:bg-slate-50 rounded-xl transition-all group">
