    ...
    <div class="lg:ml-72 w-full min-h-screen p-8">
                            ...
                            <tr>
                                ...
                                @foreach($bookInventories as $inventory)
                                    ...
                                    <td class="border border-slate-600 p-1 text-center text-xs">
                                        @if($book)
                                            @if($book->copy_number)
                                                <span class="font-bold">Copy #{{ $book->copy_number }}</span><br>
                                            @endif
                                            @if($book->status == 'lost')
        ...
        <div class="mt-6 flex gap-3 no-print">
            @if($selectedSection && $selectedSection->id)
                <a href="{{ route('teacher.books.issue', ['section' => $selectedSection->id]) }}" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                    <i class="fas fa-plus mr-2"></i>Issue Book
                </a>
                <a href="{{ route('teacher.books.return', ['section' => $selectedSection->id]) }}" class="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">
